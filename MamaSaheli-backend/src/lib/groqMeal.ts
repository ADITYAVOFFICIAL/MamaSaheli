...
if (!API_KEY) {
    console.error("CRITICAL ERROR: VITE_GROQ_API_KEY is missing. Meal/Exercise generation will be unavailable.");
...
- You are MamaSaheli, an informational assistant providing general suggestions. You are **NOT** a medical professional, doctor, registered dietitian, or certified personal trainer. Your advice is not a substitute for professional consultation.
...
`;
    ...
    return `
You are MamaSaheli, an informational assistant for pregnant users. Provide supportive, safe, general suggestions based on the user's profile while strictly adhering to safety guidelines and JSON output format.
