// MamaSaheli-backend/src/index.ts
...
import { connectFluvio, consumeTopic, disconnectFluvio } from './lib/fluvioService.ts';
import type { RecordProcessor, FluvioRecord } from './lib/fluvioService.ts'; // Import types
...
const websocketHandler: WebSocketHandler<unknown> = {
    open(ws) {
        ...
        ws.send(JSON.stringify({ type: 'info', payload: 'Connected to MamaSaheli real-time service.' }));
    },
    message(ws, message) {
