    ...
    return (
        <li className="flex items-center justify-between py-3 border-b last:border-b-0">
            <div className="flex items-start space-x-3">
                <Pill className="h-5 w-5 text-mamasaheli-primary mt-0.5 flex-shrink-0" />
...
const MedReminder: React.FC<MedReminderProps> = ({
    ...
    return (
        <Card className="border border-mamasaheli-accent/30 shadow-sm bg-white">
            <CardHeader className="bg-mamasaheli-accent/5 border-b border-mamasaheli-accent/10">
                <div className="flex justify-between items-center">
                    <CardTitle className="flex items-center text-mamasaheli-accent text-lg font-semibold">
                        <Clock className="mr-2 h-5 w-5" />Medication Reminders
                    </CardTitle>
                    <Button size="sm" variant="outline" onClick={onAddReminder} className="text-mamasaheli-accent border-mamasaheli-accent/50 hover:text-mamasaheli-accent hover:bg-mamasaheli-accent/5">
            ...
            <CardContent className="p-0"> {/* Remove padding, handled by list/item */}
                {isLoading ? (
                    <div className="flex justify-center items-center py-10">
                        <Loader2 className="h-6 w-6 text-mamasaheli-accent animate-spin mr-2" />
