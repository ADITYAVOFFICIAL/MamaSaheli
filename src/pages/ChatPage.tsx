...
const ChatPage: React.FC = () => {
  ...
  const handleExportPDF = useCallback(async () => {
      ...
      try {
          ...
          pdf.text('MamaSaheli Chat Export', pdfWidth / 2, margin, { align: 'center' });
          pdf.setFontSize(9);
          ...
          const filename = `mamasaheli-Chat-${format(new Date(), 'yyyyMMdd-HHmm')}.pdf`;
          pdf.save(filename);
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {starters.map((starter, index) => (
                          <Button
                              ...
                              variant="outline"
                              size="sm"
                              className="text-left justify-start h-auto py-1.5 px-2 border-gray-300 dark:border-gray-700 hover:bg-gray-100/70 dark:hover:bg-gray-700/60 dark:text-gray-300 text-gray-700 transition-colors hover:text-mamasaheli-primary dark:hover:text-mamasaheli-light"
                          ...
                          >
                              {starter.icon && <starter.icon className="h-3.5 w-3.5 mr-1.5 text-mamasaheli-primary/80 dark:text-mamasaheli-light/80 shrink-0" aria-hidden="true" />}
