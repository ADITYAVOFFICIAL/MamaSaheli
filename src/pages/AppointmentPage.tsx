...
const AppointmentPage: React.FC = () => {
  ...
  const { data: userProfile, isLoading: isLoadingProfile } = useQuery<UserProfile | null, Error>({
    queryKey: ['userProfile', user?.$id],
    queryFn: () => {
        if (!user?.$id) return null;
        return getUserProfile(user.$id); // Fetches the complete user profile including hospital info. [3, 5]
    },
    enabled: !!user?.$id, // Query is enabled only when the user ID is available. [2, 4]
    staleTime: 1000 * 60 * 15, // Cache profile data for 15 minutes to avoid frequent refetches.
  });
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          <div className="text-center mb-10 md:mb-12">
            <h1 className="text-3xl font-extrabold text-mamasaheli-dark sm:text-4xl tracking-tight">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                 ...
                 <CardContent className="p-6 space-y-5">
                   {isLoadingProfile ? (
                     ...
                     <Alert variant="default" className="bg-blue-50 border-blue-200 text-blue-800">
                       ...
                       <AlertTitle className="font-semibold">Primary Hospital</AlertTitle>
                       <AlertDescription className="text-xs">
                         Your appointments are associated with: <strong>{userProfile.hospitalName}</strong>.
                       </AlertDescription>
                     </Alert>
                   ) : (
                   ...
                   <Button
                     ...
                     size="lg"
                     className="w-full py-3 text-base bg-mamasaheli-primary hover:bg-mamasaheli-dark transition-colors duration-200"
                     disabled={!date || !time || isBookingLoading || !isAuthenticated || !userProfile?.hospitalName}
                     aria-disabled={!date || !time || isBookingLoading || !isAuthenticated || !userProfile?.hospitalName}
                   >
                     {isBookingLoading ? (
                       <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Booking...</>
                     ) : (
                       <><PlusCircle className="mr-2 h-5 w-5" /> Confirm Booking</>
                     )}
                   </Button>
                 </CardContent>
               </Card>
            ...
            <div className="lg:col-span-2">
              <Card className="shadow-lg border border-gray-200 rounded-lg overflow-hidden bg-white">
                ...
                <CardContent className="p-4 md:p-6 min-h-[300px] flex flex-col">
                  {isLoadingAppointments && (
                    <div className="flex flex-col justify-center items-center text-center flex-grow py-10">
                      <Loader2 className="h-10 w-10 animate-spin text-mamasaheli-primary mb-3" />
                  ...
                  {!isLoadingAppointments && errorAppointments && (
                    <div className="flex flex-col justify-center items-center text-center flex-grow py-10 bg-red-50 p-6 rounded-md border border-red-200">
                       ...
                       <p className="text-red-700 font-semibold mb-1">Error Loading Appointments</p>
