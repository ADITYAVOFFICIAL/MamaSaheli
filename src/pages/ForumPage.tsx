        ...
        <div className="flex items-start space-x-3 p-3 md:p-4 border-b dark:border-gray-700 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors duration-150">
            ...
            <div className="flex-grow min-w-0">
                <Link to={`/forum/${topic.$id}`} className="group block">
                    <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200 group-hover:text-mamasaheli-primary dark:group-hover:text-mamasaheli-accent line-clamp-2 break-words mb-1">
                        {topic.isPinned && <Pin className="inline-block h-4 w-4 mr-1.5 text-mamasaheli-accent flex-shrink-0" aria-label="Pinned" />}
...
const ForumPage: React.FC = () => {
    ...
    const renderTopicList = () => (
        <div className="space-y-6">
            <Card className="sticky top-[var(--header-height,60px)] z-10 shadow-sm border dark:border-gray-700 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <CardContent className="p-3 md:p-4">
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-4">
                        <h2 className="text-xl md:text-2xl font-semibold text-gray-800 dark:text-gray-200 flex items-center flex-shrink-0"> <MessageSquare className="mr-2 h-6 w-6 text-mamasaheli-primary" /> Community Forum </h2>
                        <Button onClick={() => { if (!isAuthenticated) { toast({ title: "Login Required", description: "Please log in to create a topic.", variant: "default" }); navigate('/login'); } else { setShowCreateTopicForm(true); } }} className="bg-mamasaheli-primary hover:bg-mamasaheli-dark w-full md:w-auto"> <PlusCircle className="mr-2 h-4 w-4" /> Create New Topic </Button>
                    </div>
            ...
            <div className="min-h-[300px]">
                ...
                {topicsLoading && topics.length > 0 && (<div className="text-center mt-6"><Loader2 className="h-6 w-6 animate-spin text-mamasaheli-primary mx-auto" /></div>)}
            </div>
        </div>
    );
    ...
    const renderTopicDetail = () => (
        <div className="space-y-6">
            ...
            {topicLoading && (<div className="text-center py-20"><Loader2 className="h-10 w-10 animate-spin text-mamasaheli-primary mx-auto" /></div>)}
            ...
            {currentTopic && !topicLoading && (
                <Card className="border-mamasaheli-primary/30 dark:border-gray-700 shadow-md">
                    <CardHeader className="bg-gray-50 dark:bg-gray-800/50 border-b dark:border-gray-700">
                        <div className="flex justify-between items-start flex-wrap gap-2">
                            <div className="flex-grow min-w-0">
                                <CardTitle className="text-xl md:text-2xl font-bold text-mamasaheli-dark dark:text-gray-100 flex items-center flex-wrap gap-2 break-words"> {currentTopic.isPinned && <Pin className="inline-block h-5 w-5 mr-1.5 text-mamasaheli-accent flex-shrink-0" aria-label="Pinned" />} {currentTopic.isLocked && <Lock className="inline-block h-5 w-5 mr-1.5 text-gray-500 flex-shrink-0" aria-label="Locked" />} {currentTopic.title} </CardTitle>
                                ...
                                <CardContent>
                                    {isAuthenticated ? (
                                        <form onSubmit={handleReply} className="space-y-3">
                                            ...
                                            <div className="text-right"> <Button type="submit" className="bg-mamasaheli-primary hover:bg-mamasaheli-dark min-w-[130px]" disabled={isReplying || isFormattingReply || !replyContent.trim() || isModerating}> {isModerating ? <><ShieldAlert className="mr-2 h-4 w-4 animate-pulse" /> Checking...</> : isReplying ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Posting...</> : <><Send className="mr-2 h-4 w-4" /> Post Reply</>} </Button> </div>
                                        </form>
                                    ) : (
                                        <div className="text-center text-sm text-gray-600 dark:text-gray-400"> Please <Link to={`/login?redirect=${location.pathname}`} className="text-mamasaheli-primary font-semibold hover:underline">log in</Link> or <Link to={`/signup?redirect=${location.pathname}`} className="text-mamasaheli-primary font-semibold hover:underline">sign up</Link> to reply. </div>
                                    )}
                                </CardContent>
                            </Card>
                        ) : (
