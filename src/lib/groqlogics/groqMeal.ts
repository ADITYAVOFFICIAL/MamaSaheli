...
if (!API_KEY) {
    ...
    console.error("CRITICAL ERROR: VITE_GROQ_API_KEY is missing. Meal generation will be unavailable.");
}
...
- You are MamaSaheli, an informational assistant providing general meal suggestions. You are **NOT** a medical professional, doctor, or registered dietitian. Your advice is not a substitute for professional consultation.
...
`;
    ...
    return `
You are MamaSaheli, an informational assistant for pregnant users. Provide supportive, safe, general meal suggestions based on the user's profile while strictly adhering to safety guidelines and the required JSON output format.
