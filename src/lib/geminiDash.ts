...
// --- Import Gemini types and the Gemini service for API calls ---
import geminiService, { AppChatMessage } from './gemini';


// --- Helper: Get Trimester Info (Unchanged) ---
...
// --- Helper: Format Profile Data for Feed Context (Unchanged) ---
...
// --- Helper: Format Recent Readings for Feed Context (Unchanged) ---
...
// --- Helper: Format Upcoming Appointments for Feed Context (Unchanged) ---
const formatAppointmentsForFeed = (appointments: (Appointment & { dateTime?: Date | null })[]): string => {
    ...
    const sortedApps = appointments
        .filter(app => app.dateTime instanceof Date && !isNaN(app.dateTime.getTime()))
...
// --- Helper: Date Formatting (Unchanged) ---
...
7.  **Handle Missing Data:** If context is limited, provide more general pregnancy tips or affirmations.


[Output Format]
-   Use Markdown for formatting (e.g., bullet points \`*\`, bold \`**\`).
-   Separate items clearly (e.g., using a horizontal rule \`---\`).
...
/**
 * Generates personalized dashboard feed content using the Gemini service.
 *
 * @param profile - User's profile.
 * @param bp - Latest BP reading.
 * @param sugar - Latest Sugar reading.
 * @param weight - Latest Weight reading.
 * @param appointments - Array of upcoming appointments.
 * @returns A Promise resolving to the generated Markdown string.
 * @throws Error if the Gemini service is unavailable or the API call fails.
 */
export const generateDashboardFeed = async (
    ...
    // Ensure Gemini service and sendMessage function are available
    if (!geminiService || typeof geminiService.sendMessage !== 'function') {
        console.error("Gemini service or sendMessage function is not available.");
    ...
    // Gemini's `sendMessage` in our setup uses a system instruction in the model config,
    // so we send a message array with the system prompt and a user prompt to kick off the generation.
    ...
    try {
        // Use the sendMessage function from our new gemini.ts service
        console.log("Requesting dashboard feed from Gemini...");
        const response = await geminiService.sendMessage(messages);
        console.log("Received dashboard feed response from Gemini.");
        return response; // Return the generated Markdown string
    } catch (error) {
        console.error("Error generating dashboard feed with Gemini:", error);
        // Re-throw the error to be handled by the calling component (e.g., DashboardPage)
        throw error;
    }
};
...
export const geminiDashboardService = {
